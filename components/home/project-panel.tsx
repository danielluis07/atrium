 ...
 * drawing set, on a paper sheet from the right. It leaves the Scene live
  ...
  return (
    <Sheet
      ...
      <SheetContent
        ...
        // under the header, which stays in view; a short slide (`DESIGN.md` § Motion)
        className="gap-0 bg-card shadow-none data-[side=right]:top-(--header-height) data-[side=right]:h-auto data-[side=right]:w-full data-[side=right]:data-ending-style:translate-x-4 data-[side=right]:data-starting-style:translate-x-4 data-[side=right]:sm:max-w-md">
        {project && (
          <>
            <SheetHeader className="gap-0 px-6 pt-6 pb-10">
              <p className={labelClass}>
                Project {formatIndex(shown + 1)} / {formatIndex(projects.length)}
              </p>
              <SheetTitle className="mt-10 font-heading text-[clamp(2rem,3vw,2.75rem)] leading-[1.05] font-light tracking-[-0.02em]">
                {project.name}
              </SheetTitle>
            </SheetHeader>
            <DataBlock
            ...
            />
            <SheetDescription className="px-6 pt-8 text-base text-foreground">{project.lede}</SheetDescription>
